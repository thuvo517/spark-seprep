...
def plan_trip(park_name):
    ...
    Instead of one big question, we break the work into steps:
      ...
      4. Find the best photography spots
...
Include a fun fact: {park['fun_fact']}"""
    ...
    # STEP 4: Photography spots
    
    print("\n[Agent] Step 4: Finding the best photography spots...\n")

    photography_prompt = f"""You are a nature photographer and travel guide for {park_name.title()} National Park.

    Based on this 2-day itinerary:
    {itinerary[:400]}

    And these top activities at the park: {', '.join(park['top_activities'])}

    Recommend 3 must-visit photography spots at {park_name.title()}. For each spot include:
    - The location name
    - The best time of day to shoot (golden hour, midday, etc.)
    - One sentence on what makes it visually unique

    Keep it practical and specific."""

    photo_spots = ask_llm(photography_prompt)
    print("--- Photography Spots ---")
    print(photo_spots)
...
Photography highlights: {photo_spots[:200]}"""

    summary = ask_llm(summary_prompt)
    print("--- Your Trip Summary ---")
    print(summary)

    # Write trip plan to file
    filename = f"{park_name.replace(' ', '_')}_trip_plan.txt"
    with open(filename, "w") as f:
        f.write(f"NATIONAL PARK TRIP PLANNER\n")
        f.write(f"Trip to: {park_name.title()} National Park\n")
        f.write("=" * 60 + "\n\n")
